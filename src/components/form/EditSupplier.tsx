...
const EditSupplier: React.FC<EditSupplierProps> = ({ supplierId, onClose, onEditSuccess }) => {
  ...
  useEffect(() => {
    const fetchSupplier = async () => {
      try {
        const response = await fetch(`${SUPPLIERS_ENDPOINT}/${supplierId}`);
        if (!response.ok) {
        ...
        setSnackbar({ open: true, message: 'Failed to load supplier data', severity: 'error' });
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onEditSuccess('Supplier updated successfully');
      onClose();
    } catch (error) {
        ...
        <CardContent>
          ...
          >
            ...
            <StyledButton
              ...
              disabled={isLoading}
            >
              {isLoading ? <CircularProgress size={24} /> : 'Update Supplier'}
            </StyledButton>
          </Box>
        </CardContent>
      </StyledCard>
